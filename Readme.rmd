...
bibliography: Bibliografia_sql.bib
csl: abnt.csl
---
...
Introdução a linguagem SQL - abordagem pratica para iniciantes. [@nield2016introdução] \
...
```{r operadores matemáticos, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
Operador <- c("+","-","*","/","%")
Descrição <- c("Soma", "Subtração",  "Multiplicação", "Divisão", "Resto da divisão")
op_mat <- data.frame(Operador, Descrição)
kable(op_mat, col.names = c("Operador","Descrição"), align = "cc",
...
```{r tabela verdade, echo=FALSE, message=FALSE, warning=FALSE}
tbl_verdade <- data.frame(p=c("V","V","F","F"),NOT_p= c("F","F","V","V"), q=c("V","F","V","F"),NOT_q= c("F","V","F","V"),
           p_AND_q= c("V","F","F","F"), p_OR_q = c("V","V","V","F"))
kable(tbl_verdade, col.names = c("P","NOT P","Q","NOT Q","P AND Q","P OR Q"),
align = "cccccc",
...
```{r operadores logicos, echo=FALSE, message=FALSE, warning=FALSE}
...
kable(op_logic, col.names = c("Operadores Lógicos", "Descrição", "Exemplo"), align = "ccc",
...
```{r funções agregação, echo=FALSE, message=FALSE, warning=FALSE}
library(base)
...
Descricao_func_tipica_groupby <- c('Calcula a media de todos os valores da coluna X (Omite valores nulos)',
               'Conta o número de valore não nulos da coluna X',
               'Conta o número registros',
               'Encontra o valor máximo da coluna X (Omite valores nulos)',
               'Encontra o valor mínimo da coluna X (Omite valores nulos)',
...
kable(funcs_agregacao, col.names = c("Função","Descrição"), align = "cc",
      caption = "Funções tipicas do GROUP BY.")
...
- Onde é possivel colocar a instrução CASE dentro de uma função de agregação, substituindo assim o uso do `WHERE`.\
...
Esses outros operadores não tem suporte no **SQLite**, porem tem nos outros banco de dados.\
  ...
  1. `RIGHT JOIN`
    - Mescla duas tabelas, uma há direita.\
    - Mantem todos os registros da tabela da direita.\
    - Diferente do `INNER JOIN`, não omite registros. Registros sem associação entre as tabelas recebe valor `NULL`.\
  
  2. `OUTER JOIN`
    - `OUTER JOIN` é um operador de associação externa completa.\
...
## Associando várias tabelas
  ...
  1. Associação de diversas tabelas `INNER JOIN`\
    - Associa três ou mais tabelas atraves de colunas *CHAVES*, entre elas.\
  ...
  2. Agrupando `JOIN`s\
    ...
    - Por conseguencia é possivel usar as funções de agrupamento para conseguir novas informações.\
    - Sintaxe:\
    ```
    SELECT
    coluna1,
...
- Uma dica para o design é fazer o diagrama RE (relaciomento de entidade), no qual exibe as tabelas e como elas estão relacionadas.\
...
1. Chave Primaria:
  ...
  - Chave primaria aumenta a eficienca nas consultas do software de banco de dados.\
...
- regras:\
  ...
  - **DEFAULT**
    - Determina um valor default para o registro, muito util para tipo BOOLEANO.\
    - Sintaxe:\
    **DEFAULT** (0)\
...
- Podemos consultar uma *view* como se ela fosse uma tabela, ou seja, chamar a *view*, apelicar:\
    - **SELECT**
    - **WHERE**
    - **CASE** 
    - ...
...
- Multiplos **INSERT** simultâneos\
  ...
  - Processo muito útil para inserções automatizada atraves de linguagens de programação, como:\
    - Python\
    - R\
    - Java\
    - ...\
...
- Chaves externas\
  - Se for inserido um registro em que a *chave externa* esteja errada, registro orfão, o registro não será aceito.\
...
Obs.: No **MySQL** a melhor forma de DELETAR todos os registro s de uma tabela é pela instrução **TRUNCATE TABLE**.\
Ex. Sintaxe:\
...
- **UPDATE**
  ...
  - Pode moficicar diversos registros de uma vez por meio de uma função.\
  - Sintaxe:\
